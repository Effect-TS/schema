...
describe.concurrent("required", () => {
  ...
  it("struct", async () => {
    const transform = S.required(S.struct({
      a: S.optional(S.number.pipe(S.greaterThan(0)))
    }))
  ...
  it("tuple/ e?", async () => {
    // type A = [string?]
    // type B = Required<A>
    const transform = S.required(S.tuple().pipe(S.optionalElement(S.number)))
  ...
  it("tuple/ e + e?", async () => {
    const transform = S.required(S.tuple(S.number).pipe(S.optionalElement(S.string)))
